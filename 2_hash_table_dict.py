...
def count_hash_table(C, hash_table, k):
    # deleting transaction from hash table according to k
    for transaction, ids in list(hash_table.items()):
        if len(ids) < k:
            del hash_table[transaction]

    C_count = {ids: 0 for ids in C}
    # counting if itemset in C occur in each transaction by checking combination in dict keys
    for transaction in hash_table.values():
        for ids_comb in combinations(transaction, k):
            ids_comb = frozenset(set(ids_comb))
            if ids_comb in C_count.keys():
                C_count[ids_comb] += 1
...
# create a hash table for each itemsets by their transaction id
...
def apriori(hash_table, min_sup):
    ...
    while(current_L):
        ...
        Ck_count = count_hash_table(Ck, hash_table, k)
